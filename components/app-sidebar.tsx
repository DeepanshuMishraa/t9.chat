      ...
      <SidebarContent className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4">
          {threads?.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {threads?.map((thread) => {
                return (
                  <Link href={`/chat/${thread._id}`} key={thread._id} className="w-full py-4 px-5 flex items-center gap-4 justify-start text-md hover:bg-primary/10 transition-all duration-300 rounded-lg">
        ...
        <SidebarFooter className="border-t bg-background p-4">
          {isLoading ? (
            ...
            <Button variant="outline" size="lg" className="flex items-center justify-center gap-2 w-full py-6">
              <Image
