"use client"

import Link from "next/link";
import { ArrowLeftIcon } from "lucide-react";
import { Button } from "@/components/ui/button";
import { authClient } from "@/lib/auth-client";
import { useRouter } from "next/navigation";

export default function AuthPage() {
  const router = useRouter();
  return (
    <div className="relative isolate min-h-[100svh] md:min-h-[100svh] flex items-center justify-center">
      <div className="absolute left-4 top-4 md:left-6 md:top-6">
        <Link href="/" className="inline-flex items-center gap-2 text-sm md:text-base text-foreground/80 hover:text-foreground transition-colors">
          <ArrowLeftIcon className="size-4 md:size-5" />
          Back to Chat
        </Link>
      </div>
      <div className="pointer-events-none absolute inset-0 -z-10 [mask-image:radial-gradient(ellipse_at_center,black,transparent_65%)]" />

      <div className="mx-auto w-full max-w-xl px-6 text-center">
        <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">
          Welcome to <span className="text-primary font-bold">T9.chat</span>
        </h1>
        <p className="mt-3 text-sm md:text-base text-muted-foreground">
          Bring your own keys and start chatting with your own data.
        </p>

        <div className="mt-8 md:mt-10">
          <Button
            size="lg"
            className="w-full md:w-[28rem] max-w-full mx-auto h-12 md:h-14 text-base md:text-lg rounded-xl bg-primary/30 hover:bg-primary/40 border border-primary/30 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur"
            onClick={() => {
              authClient.signIn.social({
                provider: "google",
                fetchOptions: {
                  onSuccess: () => {
                    router.push("/");
                  },
                  OnError: (e: any) => {
                    console.error(e);
                  }
                }
              })
            }}
          >
            <svg className="mr-3 h-6 w-6" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"></path><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"></path><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"></path><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"></path></svg>
            Continue with Google
          </Button>
        </div>

        <p className="mt-5 text-xs md:text-sm text-muted-foreground">
          By continuing, you get to enjoy multimodal chats with your own keys.
        </p>
      </div>
    </div>
  );
}


